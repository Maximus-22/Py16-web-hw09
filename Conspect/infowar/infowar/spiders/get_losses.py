...
def get_next_link():
    with open('inner_links.json', 'r', encoding='utf-8') as fd:
        result = json.load(fd)
...
class GetLossesSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        # Особливiсть павукiв Scrapy - виддача даних через [yield], тому тут нiяких словникiв не може
        # бути, то ж працюємо за допомогою [dict()]
        parse_elements = {}
        ...
        for li in content:
            ...
            for casual in casualties:
                # зайди в усi теги униз та забери вси поля з атрибутами <text>, оскiльки таких полiв декiлька
                # потрiбно вже використати функцiю extract()
        ...
        # цей цикл для переходу на наступну сторiнку та обробки її контенту, отримаємо вiдповiдь <response>
        # та закидуємо її до [parse()]
